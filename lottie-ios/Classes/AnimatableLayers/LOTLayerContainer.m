...
- (void)_setImageForAsset:(LOTAsset *)asset   {
  if (asset.imageName) {
      ...
      //MARK:判断有没有遵循自定义协议,如果遵循,走自定义方法
      if (asset.assetBundle && [asset.assetBundle conformsToProtocol:@protocol(LOTBundleDelegate)]) {
          id<LOTBundleDelegate> cache = asset.assetBundle;
          image = [cache imageForKey:asset.imageName];
      } else {
...
//        // Contents look like a data: URL. Ignore asset.imageDirectory and simply load the image directly.
//        NSURL *imageUrl = [NSURL URLWithString:key];
//        NSData *imageData = [NSData dataWithContentsOfURL:imageUrl];
//        image = [UIImage imageWithData:imageData];
//    } else if (asset.rootDirectory.length > 0) {
//        NSString *rootDirectory  = asset.rootDirectory;
